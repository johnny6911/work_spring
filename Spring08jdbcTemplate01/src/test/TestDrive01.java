...
public class TestDrive01 {
	public static void main(String[] args) {
		...
		// 2-2. JdbcTemplate 객체를 이용하여 SQL query문 실행
		// 클래스의 타입을 이용해서 빈 객체를 가져온 경우
		JdbcTemplate template = context.getBean(JdbcTemplate.class);
	
		// SQL문 만들기: emp 테이블의 행의 개수를 가져오는 SQL문
		String sql = "SELECT COUNT(*) FROM emp"; // 현재 직원 14명 입력했으므로 14라고 뜸(1: 14)
		
		//int count = template.queryForInt(sql);	// Spring 3버전에서 사용하던 방식
		...
		count = template.queryForObject(sql, Integer.class, 20); //부서번호가 20번인 직원들의 수, 답 : 5
		System.out.println("4: " + count);
		
		// 4.
		sql = "SELECT ename FROM emp WHERE empno =?";
		String name = template.queryForObject(sql, String.class, "7900"); //7900번에 해당하는 직원을 출력
		 ...
		 *  첫번째 파라미터: SQL문
		 *  두번쨰 파라미터: 물음표에 해당하는 값
		 *  현재 모든 직원들의 정보가 empList에 들어가있음
		 */
		List<Map<String, Object>> empList = template.queryForList(sql, 20); // 부서번호가 20번인 모든직원들의 정보를 출력하는 쿼리문
		
		/* 
		 * 7-3. Map은 각 행을 의미, 각 행을 모두 담고 있어야 하기 때문에
		 * Map 타입의 List로 사용하는 것이다.
		 * 첫번째 for문은 List에서 각 행 하나를 끄집어 낸 것이다.
		 * List에서 각 직원의 정보(Map 하나씩) 끄집어낸것
		 */
		for (Map<String, Object> item : empList) {
			
			/*
			 *  7-4. 두번째 for문은 6번 예제와 같이 key값을 이용하여
			 *  각 행의 Map에서 value값을 끄집어 낸 것이다
			 */
