...
public class Spring3AjaxRestController {
	...
	public void addCar(@RequestBody Car car) { // 리케스트바디에 있는 내용이 car 객체에 저장됨
	...
	public Map modifyCar(@RequestBody Car car) {
		...
		for (int i = 0; i < cars.size(); i++) {
			if (car.getName().equalsIgnoreCase(cars.get(i).getName())) {
				cars.get(i).setNumber(car.getNumber());
				
				resultMap.put("message", "차량수정 성고그");
				return resultMap;
			}
		}
		resultMap.put("message", "차량수정 성고그");
	...
	public Map<String,Object> removeCar(@PathVariable("name") String name){
		...
		cars.add(new Car("BMW", 20너 1234));
		cars.add(new Car("모닝", 20너 1234));
		
		// 데이터베이스에서 삭제할 차량 검색
		Map<String, Object>	 resultMap = new HashMap<String, Object>();
		for (int i = 0; i < cars.szie(); i++) {
			if (name.equals(cars.get(i),getName())) {
				cars.remove(i);
				resultMap.put("message", "차량 삭제제 성공");
				return resultMap;
			}
		}
		resultMap.put("message", "차량 삭제제 실패");
		return resultMap;
	}
