...
public class KakaoPayWebController {
	...
	public String ready(@RequestParam Map<String, String> order, 
		...
		/*
		 * 우리 페이지주소를 보내주는 이유는 결제준비 단계에서
		 * 우리페이지에서 카카오페이지로 넘어갔기 때문에
		 * 카카오페이지에서 결제 처리 후 다시 우리 페이지로 넘어오려면
		...
		headers.add("Authorization", "KakaoAK " + "fda53bf0b6c0b95a6ff049b89f3d2609");
		...
		HttpEntity<MultiValueMap<String, String>> request = 
				new HttpEntity<MultiValueMap<String, String>>(params, headers);//httpEntity라는 객체 하나에 담음
		...
		/* 
		 * 결제승인 페이지에서에서 사용할 정보를 Session 객체에 담기
		 * (결제 고유번호, 가맹점 주문번호, 가맹점 회원ID)
		 * Session 객체를 이용하는 이유는 사용자마다 고유의 Session 객체를 가지고 있어서
		 * 다른 사용자의 결제과정에 영향을 끼치지 않기에 사용
		 */
		session.setAttribute("tid", resultMap.get("tid")); //100명이 접속하면 세션이 100개가 생김, 다른 섹션에 영향을 끼치지 않기위해
