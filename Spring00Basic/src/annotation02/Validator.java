...
public class Validator {
	...
	public boolean validate(Object obj) throws IllegalArgumentException, IllegalAccessException {
		// 검증 기준에 통화했는지 여부를 저장하는 변수
		boolean pass = true;
		
		// 현재 검증하려는 obj의 toString을 호출하여 정보출력
		...
		for (Field field : fields) {
			...
			if (field.getAnnotation(Validation.class) != null) {
				...
				Validation anno = field.getAnnotation(Validation.class); // 애노테이션 객체 생성하는법
				...
				// 5. 만약 필드의 타입이 String 일 경우
					...
					// 7. 만약 required 값이 true이고 valStr 값이 없을경우
