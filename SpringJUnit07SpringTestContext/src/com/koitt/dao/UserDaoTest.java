...
public class UserDaoTest {
	 ...
	 */
	@Before
	public void setUp() { // 셋업은 테스트+마다 실행된다
		...
		System.out.println("컨텍스트 객체의 주소값: " + this.context);
		System.out.println("UserDaoTest 객체의 주소값: " + this);
	}
	...
	public void addAndGet() throws SQLException { // JUnit 테스트 메소드는 반드시 public으로 선언
		...
		assertThat(userget2.getName(), is(user2.getName())); //assertThat은 JUnit 라이브러리를 추가해야만 사용할수있다
	 ...
	 * 						에외가 던져진다					EmptyResultDataAccessException.class)
	 */
	 ...
	 * 	   테스트 메소드를 모두 찾는다
	 * 2. 테스트 클래스의 객체를 하나 만든다. (객체를 매번 새로 만드는 이유는 독립적으로
	 * 	   실행됨을 확실히 보장해주기 위해)
